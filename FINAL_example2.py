...
def log_prior(theta):
	...
	temp = theta[:,:]
	temp1 = temp[0]
	if(all(temp1[0:] > 0) and all(temp1[0:] < 1)):
		return 0
	else:
...
def log_posterior(theta):
	...
	e = np.array([ 3.6, 3.9, 2.6, 3.4, 3.8, 3.8, 2.2, 2.1, 2.3, 3.8, 2.2, 2.8, 3.9, 3.1, 3.4, 2.6, 3.4, 3.7, 2.0, 3.5])
	#print(theta)
	return log_prior(theta) + log_likelihood(theta,x,y,e,50)
...
domain = [{'name' : 'var1', 'type' : 'continuous' , 'domain' : (30,31) , 'dimensionality' : 1},     # this is for the intercept
	  {'name' : 'var2', 'type' : 'continuous' , 'domain' : (0.4,0.5) , 'dimensionality' : 1},   # this is for the slope
	  {'name' : 'var3', 'type' : 'continuous' , 'domain' : (0.4,1.5) , 'dimensionality' : 20}]  # these are the values of 'g' which are nuisance parameters
		

xfit = np.linspace(0,100)
myBopt = BayesianOptimization(f=log_posterior, domain = domain, initial_design_numdata = 4, acquision_type = 'EI', exact_feval = 'True')
myBopt.run_optimization(max_iter = 10, eps = 1e-6)
